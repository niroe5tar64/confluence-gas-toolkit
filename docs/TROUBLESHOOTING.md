...
本ツールキットでは社内API利用ルールに従い、Confluence REST API 呼び出しで **503** のときは 10 秒待って 1 回だけリトライ、**401** のときは認証情報（PAT）を再取得して 1 回だけリトライします。その他の 4xx/5xx や接続断・タイムアウトではリトライしません。
...
### Bandwidth quota exceeded（帯域制限超過）

**症状**: Confluence の検索 API 呼び出しで `Bandwidth quota exceeded` エラー

**原因**: Confluence Server/Data Center の API 帯域制限を超えた。以下が重なると発生しやすいです。

- ジョブの実行間隔が短い
- 監視対象のページ数が多く、ページネーションで連続リクエストが発生している
- 複数ジョブが同時刻付近で実行されている

**解決策**:
1. **コード側の対策**（本ツールキットで実施済み）  
   - ページネーションのリクエスト間に 400ms の待機を入れ、短時間の連続呼び出しを抑制
2. **運用側の対策**  
   - GAS のトリガー間隔を少し空ける（例: 15 分 → 20 分）  
   - 監視対象を `confluence-page-configs` で必要最小限に絞る  
   - Confluence 管理者に帯域制限値の確認・緩和を相談する
